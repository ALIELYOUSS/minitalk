...
/*   Updated: 2025/03/19 05:45:15 by alel-you         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minitalk.h"

static void    msg_received(int signum, int pid)
{
    if (signum == SIGUSR1 && kill(pid, SIGUSR1) == -1)
    {
       ft_putstr_fd("stoped 😩!!\n", 1);
       exit(0);
    }
...
void    handler(int signum, siginfo_t *info, void *none)
{
    ...
    {
        if (!c)
            msg_received(SIGTERM, info->si_pid);
        if (c < 127)
            write(1, &c, 1);
        else
            print_unicode(c);
